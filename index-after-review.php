...
$store = new StoreHandler(
    ...
    new PasswordEncryptor("master-password")
);
/*ВАЖНЫЙ МОМЕНТ
...
PasswordManager - отвечает за бизнес логику (он оркестрирует классы, выводит список команд и обрабатывает их) (это много для одного класса, но чуть позже я расскажу как это красиво разобрать)
    использует Store чтобы получить список паролей и модифирует их с помощью Store
Store - использует Filesystem чтобы получить фаил и расшифровывает его с помощью Encryptor
    когда человек выходит операция с файлом закончена вызывается Store::persist который с помощью Encryptor зашифрует фаил и с помощью Filesystem запишет зашифрованное содержимое в фаил
Filesystem:
...
Переименуй классы:
 - PasswordEncryptor -> Encryptor, функции encrypt и decrypt. Это класс общего назначения и он не должен знать о существовании паролей вообще, он может использоваться в других целях
*/
